...
- [OpenAI Agents API - a hosted agent harness with no harness fee and an admitted metering gap](https://developers.openai.com/api/docs/guides/agents-api/observability) - OpenAI's hosted Codex harness, in public beta since 2026-09-10, bills every root-agent and subagent model call at ordinary token rates with no harness fee published, and its own docs state the usage fields cannot determine the exact charge where cache-write pricing applies. (checked 2026-09-14) ![tool: proprietary](https://img.shields.io/badge/tool-proprietary-blue?style=flat-square)
...
- [Cached-input discounts - the ~90%-off lever behind cache-accounting](https://platform.claude.com/docs/en/build-with-claude/prompt-caching) - Cache-read pricing discounts input tokens by about 90%: the biggest lever on an agentic bill, since input is roughly 85% of session cost. (checked 2026-09-14)
- [ChatGPT subscription tiers and Codex CLI bundling/pricing (2026)](https://learn.chatgpt.com/docs/pricing) - OpenAI bundles Codex into every ChatGPT tier from Free through Pro (from $100/mo), differing by rate-limit multiplier; the credit rate card now prices GPT-5.6 Sol, Terra, and Luna. (checked 2026-09-14)
...
- [Claude Fable 5.1 - same headline price, a 4x steeper cache-hit discount](https://www.anthropic.com/claude-fable-and-mythos-5-1) - The full rate card for Claude Fable 5.1 and its Mythos 5.1 twin, launched 2026-09-01 at $10 input / $12.50 5m cache write / $20 1h cache write / $0.25 cache hit / $50 output per MTok, with the 1M context window at standard pricing, batch at 50%, and no fast-mode tier. (checked 2026-09-14)
- [Claude's tokenizer vocabulary shrank, and plain English got about 46% more expensive](https://tokencontributions.substack.com/p/on-the-biology-of-claudes-tokenizer) - Claude's tokenizer vocabulary shrank from 48,792 pieces to 15,283 between model generations, and the same plain English text now costs about 46% more tokens - a bill increase with no line on any price card. (checked 2026-09-14)
...
- [DeepSeek V4.1-Flash - a 4-7x cut, a 35-hour peak window, and a retirement that was called off](https://api-docs.deepseek.com/quick_start/pricing) - DeepSeek's rate card as of 2026-09-14: V4.1-Flash at $0.15 cache-miss input / $0.003 cache hit / $0.60 output per 1M off-peak, V4-Pro at roughly 4-7x that, every rate doubling during a 35-hour weekly peak window, and the announced V4-Pro retirement reversed before it took effect. (checked 2026-09-14)
...
- [LLM token pricing dimensions - the structure of a token bill](https://platform.claude.com/docs/en/about-claude/pricing) - This maps out how frontier LLM APIs meter and price tokens, read straight off the two largest providers' pricing pages, Anthropic and OpenAI. (checked 2026-09-14)
