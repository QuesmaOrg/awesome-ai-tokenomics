...
- [MCP tool-schema token overhead (spec issue)](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/2808) - A tool-by-tool measurement via Anthropic's count_tokens API filed against the MCP spec: heavy tool schemas cost ~1,000 tokens each, four independent measurements in the thread average ~123 tokens per tool, and 20-30 registered tools occupy 15-30 KB of context before the first user message. ![report](https://img.shields.io/badge/report-555?style=flat-square)
...
- [LoCoMo](https://github.com/snap-research/locomo) - Snap Research's very-long-term conversational-memory benchmark; the canonical dataset that Mem0, Zep, and Supermemory all cite, kept as a background instrument. ![bench](https://img.shields.io/badge/bench-555?style=flat-square) ![last commit](https://img.shields.io/github/last-commit/snap-research/locomo?style=flat-square&label=)
...
- [Antigravity CLI first-turn context overhead](https://github.com/google-gemini/gemini-cli/discussions/27307) - Antigravity CLI's own /context command reported 24.3K tokens consumed on turn one of a fresh session: 7.8K of system prompt plus 15.1K of system tools against 2 tokens of user input. One self-reported session; the tools figure has no second source. ![report](https://img.shields.io/badge/report-555?style=flat-square)
...
- [Codex CLI exec-mode token baseline](https://gist.github.com/alexfazio/359c17d84cb6a5af12bac88fa1db9770) - An 81-experiment trace set with raw JSONL usage events: a minimal Codex CLI exec prompt costs 8,497 input tokens at a 99.4% cache-hit rate, with per-flag ablations pricing each feature's contribution to the fixed prefix. ![data](https://img.shields.io/badge/data-555?style=flat-square)
- [GitHub Copilot CLI fixed request overhead](https://github.com/github/copilot-cli/issues/2627) - A pass-through proxy capture on Copilot CLI 1.0.43 measured about 16,800 tokens of fixed payload per request (a 36.5 KB system message plus 29 KB of schemas for 21 tools) against roughly 100 tokens of user content; the documented tool flags removed none of it. ![report](https://img.shields.io/badge/report-555?style=flat-square)
