...
- [snip](https://github.com/edouard-claude/snip) - A single-binary Go CLI proxy that filters dev-command output before it reaches a coding agent's context, with its 132 filters shipped as declarative YAML pipelines in the repository rather than compiled code, so what each one keeps or drops is readable and unit-tested per filter. Its 60-90% headline is project-measured token reduction, not measured cost. ![tool: MIT](https://img.shields.io/badge/tool-MIT-blue?style=flat-square) ![last commit](https://img.shields.io/github/last-commit/edouard-claude/snip?style=flat-square&label=)
