...
**TL;DR:** five different shapes all get called "the LiteLLM pattern" or "run Claude Code cheap," and they solve different problems. Picking the wrong one means either the wrong integration effort or the wrong outcome. Licenses and repo facts below were checked against the GitHub API on 2026-07-18 (the control-plane section: 2026-07-27).
...
**Example:** [numman-ali/cc-mirror](https://github.com/numman-ali/cc-mirror) - MIT: `npx cc-mirror quick --provider <p> --api-key <k> --model-sonnet "p/model" --model-opus "p/model"`. One npx command, no proxy process to run. ![last commit](https://img.shields.io/github/last-commit/numman-ali/cc-mirror?style=flat-square&label=)
...
**Example:** [NadirRouter/NadirClaw](https://github.com/NadirRouter/NadirClaw) - **PolyForm Noncommercial License 1.0.0**: a router-with-classifier proxy, with two caveats. Its published benchmark figures (RouterBench AUROC 0.961, the RouterArena #5 claim) describe the paid Nadir Pro classifier - the project's own README says the free OSS classifier is a simpler binary centroid with no benchmark attached. And it is **not free for commercial use**: confirm the license before adopting it at a company. ![last commit](https://img.shields.io/github/last-commit/NadirRouter/NadirClaw?style=flat-square&label=)
...
**Example:** [musistudio/claude-code-router](https://github.com/musistudio/claude-code-router) - MIT: 36,211 stars and a same-day last commit at the 2026-07-27 check. Caution: routing scripts run as fully trusted code with access to your environment variables and request headers, including credentials - only use scripts you wrote yourself. ![last commit](https://img.shields.io/github/last-commit/musistudio/claude-code-router?style=flat-square&label=)
